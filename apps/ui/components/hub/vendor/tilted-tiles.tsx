...
/*
 * React Bits Pro — "Tilted Tiles".
 * https://pro.reactbits.dev/docs/components/tilted-tiles
 * Registry item: @reactbits-starter/tilted-tiles-tw
 *
...
const edgeMask = (top: number, bottom: number) => {
  ...
  if (top > 0) {
    FALLOFF.forEach(([at, alpha]) => {
      stops.push(`rgb(0 0 0 / ${alpha}) ${((at ?? 0) * top).toFixed(3)}%`);
    });
  } else {
  ...
  if (bottom > 0) {
    [...FALLOFF].reverse().forEach(([at, alpha]) => {
      stops.push(
        `rgb(0 0 0 / ${alpha}) ${(100 - (at ?? 0) * bottom).toFixed(3)}%`
      );
    });
  } else {
