 ...
 * https://pro.reactbits.dev/docs/components/scroll-stack
 * Registry item: @reactbits-starter/scroll-stack-tw
 *
 * Kept as source rather than installed through the registry, which would need
 * components.json and the CLI. Two changes from the published component:
 *   - `cn` came from "@/lib/utils", which this app does not have; replaced with
 *     the local join below.
 *   - DEFAULT_ITEMS pointed at Unsplash for its own demo; same five cards, kept
 *     as-is. Our own cards are passed as children and do not use them.
 *
 * Otherwise whole, including the built-in `Face` layout, so re-syncing against a
 * later release stays a diff rather than a merge.
 */
import {
  Children,
  isValidElement,
...
/** Local stand-in for the `cn` this was written against. */
      ...
      >
        ...
        >
          {cards.map((card, index) => (
            <div
              ...
              className="absolute inset-0 [will-change:transform,opacity] [backface-visibility:hidden] [transform-style:preserve-3d]"
              style={{ zIndex: index }}
            >
              {custom.length > 0 ? (
                (card as ReactNode)
        ...
        {(showProgress || showCounter) && count > 1 && (
          <div className="pointer-events-none absolute inset-x-0 bottom-6 flex items-center justify-center gap-4 px-6 sm:bottom-8">
            ...
            {showCounter && (
              <span className="text-[11px] font-medium tracking-widest tabular-nums opacity-60">
...
const Face = ({
  ...
  >
    ...
    <span
      className={cn(
        "absolute top-5 right-5 text-[11px] font-medium tracking-widest tabular-nums sm:top-7 sm:right-7",
    ...
    <div className="relative flex flex-col gap-3 p-6 sm:gap-4 sm:p-9">
      ...
      {item.title && (
        <h3
          className={cn(
            "max-w-[22ch] text-2xl leading-[1.15] font-medium tracking-tight text-balance sm:text-3xl md:text-4xl",
