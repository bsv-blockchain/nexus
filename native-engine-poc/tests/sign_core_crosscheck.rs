//! Sign-core cross-check: bsv-rs `PrivateKey::sign` (k256 0.13.x, the engine's
//! sign core per the design doc) must emit DER byte-identical to bitcoin-core
//! libsecp256k1 (rust-secp256k1 =0.31.1 — the native-secp-poc core already
//! proven byte-exact vs @bsv/sdk@2.1.6 over 155,148 fuzz cases).
//!
...
/// sha256d(preimage), and P(sha256d output >= n) ≈ 2^-128. This test pins the
/// asymmetry so a future k256/libsecp bump that CHANGES it is noticed.
#[test]
